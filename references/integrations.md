...
These add-ons are **references, not bundled dependencies**. Setup does not install, activate, authenticate, or expand permissions for any entry. Review the current source, data boundary, and side effects before opting in. `listed` and `experimental` do not mean the integration has been verified end to end.

| Integration | Kind | Maturity | Writes | Publishes | Destructive | Last verified |
|---|---|---|---:|---:|---:|---|
| [Agent Skills](https://github.com/conorbronsdon/agent-skills) | `skill_catalog` | verified | Yes | No | No | 2026-08-15 |
...
- **Writes / external effects:** Chosen skill files in an agent-specific project or user directory
- **Confirmation:** Review the selected skill and destination before installation or update.
- **Risk tags:** `external-install`, `project-files`
...
- Install, diff, and update explicitly selected skills
...
- **Supported agents:** `claude_code`, `codex`, `generic`
...
- **Writes / external effects:** Local workspace state and session files after workflow approval
