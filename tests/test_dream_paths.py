...
class DreamMemoryPathTests(unittest.TestCase):
    ...
    def helper(self, *args: str, cwd: Path | None = None) -> subprocess.CompletedProcess[str]:
        return run(["python3", str(HELPER), *args], cwd or self.repo, check=False)

    def write_artifact(self, proposals: list[dict[str, object]] | object) -> Path:
    ...
    def test_merge_rejects_bad_index_tombstones_bool_and_survivor_collision(self) -> None:
        proposal = {
            ...
            "evidence": ["alpha", "beta"],
            "merged_body": "merged",
            "index_changes": {"remove": ["old"], "add": "new"},
