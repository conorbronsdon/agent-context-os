...
- **Half-finished archive.** A file listed in `ARCHIVE.md` that is still sitting in the memory root is a resumable crashed run, whether or not its `archived:` stamp was already written. Action: `archive` — **not** `modify`. `.claude/commands/dream-apply.md` classifies completion from the filesystem: destination present + root absent + one row is complete; root present + one row resumes without appending a duplicate row, reuses that row's date, and finishes stamp + move + repoint + index-drop. Root with no row is a fresh archive. Duplicate rows, root/destination collisions, or destination without a row stop for manual review. Confidence: high (mechanical).
