...
### Changed
- `/content-shipped` + `content/log.md` — the command wrote one format and the file it writes to documented another. The command told you to prepend a `## [DATE] — [Title]` section; `content/log.md` shipped a commented-out template for the same shape. Both now describe one table row per piece: `| Date | Type | Title / Description | Platform | Link | Notes |` — a fixed column order is diffable, scannable at a glance, and lets a summarizer group by Type without parsing prose. The command gains explicit **Date** and **Notes** fields, a `TBD` convention for a link that isn't live yet, and says where the row goes (last line of the *table*, not end of file). `content/log.md` no longer claims "Newest first": the command appends, backfilled entries land out of sequence either way, and the Date column is the real sort key.
