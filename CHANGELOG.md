...
### Changed
- `commands/{start,end,update,today,reconcile,recover}.md` + `skills/skill-creator/SKILL.md` — republished from the shared core after it ingested its standalone upstreams (core @ `7ae9852`). What rides in: a `workspace.yaml` config layer with per-file staleness thresholds (`/start`, `/update`, `/today`), a decision log with a rejected-alternatives column and a git safety check (`/end`), "prefer evidence of intent over recency" (`/reconcile`), "recent activity means *unknown*, not orphaned" plus a working dirty-worktree removal path (`/recover`), and a never-silently-clobber-an-existing-`SKILL.md` guard (`skill-creator`). `x-source-version` restamped on all seven; `recover` keeps its downstream `allowed-tools` declaration.
