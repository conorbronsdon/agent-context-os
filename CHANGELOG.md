...
### Changed
- `commands/{start,end,update,today,reconcile,recover}.md` + `skills/skill-creator/SKILL.md` — republished from the shared core after it ingested its standalone upstreams (core @ `7ae9852`). What rides in: a `workspace.yaml` config layer with per-file staleness thresholds (`/start`, `/update`, `/today`), a decision log with a rejected-alternatives column and a git safety check (`/end`), "prefer evidence of intent over recency" (`/reconcile`), "recent activity means *unknown*, not orphaned" plus a working dirty-worktree removal path (`/recover`), and a never-silently-clobber-an-existing-`SKILL.md` guard (`skill-creator`). `x-source-version` restamped on all seven; `recover` keeps its downstream `allowed-tools` declaration. The republished `skill-creator` links a pinned frontmatter-spec snapshot, so `skills/skill-creator/references/claude-code-frontmatter.md` now ships alongside it (verbatim from the canonical agent-skill-builder copy).
...
- `/context` — adds a `tags:` frontmatter convention with tag-first ranking, a ranked output with match-type markers and line counts, and a "first 30 lines if >3 selected" efficiency pass.
