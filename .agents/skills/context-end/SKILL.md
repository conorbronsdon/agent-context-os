...
Create a reviewed JSON payload under `<ContextRoot>/.context-os/inputs/`. Write each lifecycle fact as one self-contained sentence that names its subject; avoid label-prefixed fragments such as `Atlas: ...` and sentences that combine multiple facts. This applies to handoff prose as well as the payload arrays. The payload contains:
