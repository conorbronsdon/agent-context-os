...
- Keep exactly one `**Last Updated:**` line in `current.md`. Move its previous value into `<state_dir>/current-log.md` as a separate newest-first line under `# current.md update log`, then replace it with today's value.
